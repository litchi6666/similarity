...
from rnnlm.data_utils import read_words, build_vocab


def to_words(sentence, words):
    """ words is a List contains all words of the file
        sentence may equals to [111,222,333],and sentence is a list
        rer
    """
...
def valid_data():
    ...
    x_train, y_train = gen_data(train_data, batch_size=5, shuffle=True)

    print(x_train.shape)
    print(y_train.shape)

    print(to_words(x_train[1, 1], words))
    print(words[np.argmax(y_train[1, 1])])

    print('='*20)
    print(to_words(x_train[0, 0], words))
    print(to_words(y_train[0], words)[0])
    print(to_words(x_train[0, 1], words))
    print(to_words(y_train[0], words)[1])
    print(to_words(x_train[0, 2], words))
    print(to_words(y_train[0], words)[2])
    print(to_words(x_train[0, 3], words))
    print(to_words(y_train[0], words)[3])
    print(to_words(x_train[0, 4], words))
    print(to_words(y_train[0], words)[4])
