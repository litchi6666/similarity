...
def get_title_vec(sentence_with_seg, senten_id):
    """
    标题向量的计算
...
def get_count_matrix():
    count_matrix_baidu = CountVectorizer().fit_transform(df['baidufc'])


def count_tf_idf(sentence, word, matrix):
    """
    计算tfidf
    :param sentence: 句子id
    :param word: 某个词的id
    :param matrix: 文档-词频矩阵
    :return:
    """
    row = matrix[sentence, :]
    sum_word_of_sentence = 0
    for times in row:
        sum_word_of_sentence += int(times)
    tf = row[word]/sum_word_of_sentence

    column = matrix[:word]
    sum_word_sentence_of_crops = 0
    for times in column:
        if int(times) > 0:
            sum_word_sentence_of_crops += 1
    idf = math.log(len(sentence)/(sum_word_sentence_of_crops+1), 2)
...
if __name__ == '__main__':
    try:
        ...
        for i in range(len(df['video_name'])):  # 循环每一行，根据词向量生成句子向量 i为句子的索引
